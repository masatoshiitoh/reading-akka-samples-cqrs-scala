...
object Main {
   ...
   */
    // cassandraはファイルに保存しているので、再起動後も前の値が参照できる
  ...
  // createTablesは startNodeの中で呼ばれる。
  def createTables(system: ActorSystem[_]): Unit = {
    ...
    // https://docs.datastax.com/ja/dse/5.1/cql/cql/cql_reference/cql_commands/cqlCreateKeyspace.html
    val keyspaceStmt =
      """
    ...
    // projectionとは？？
    // https://doc.akka.io/docs/akka-projection/current/overview.html
    val offsetTableStmt =
      """
